...
function AddNewReport({
  ...
  const handleSave = async () => {
    try {
      const mylab = labs.filter((data) => data.id === state.type);
      const payload: any = {
        ...state,
      ...
      if (isEditing) {
        ...
        delete payload.labTechnician;
        delete payload.patient;
        delete payload.headDoctor;
        await dispatch(updateLaboratoryReport(payload));
      } else {
        await dispatch(createLaboratoryReport(payload));
      }
      listLaboratoryReport();
      setVisible(!visible);
    } catch (err) {
      console.log(err);
    }
