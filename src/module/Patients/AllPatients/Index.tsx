...
import { allPatients, inPatients, outPatients } from "../patients.mock";
function AllPatients() {
  const [selectedIndex, setSelectedIndex] = useState(0);
  const [selectedArr, setSelectedArr] = useState(allPatients);
  ...
  const handleTabClick = (index: number) => {
    ...
    if (index === 1) {
      setSelectedArr(inPatients);
    } else if (index === 2) {
      setSelectedArr(outPatients);
    } else if (index === 0) {
      setSelectedArr(allPatients);
    }
  };
  ...
  return (
    <div className="patients__table__container">
      ...
      <div className="table__conatiner">
        <PatientsTable data={selectedArr} />
      </div>
    </div>
  );
}
